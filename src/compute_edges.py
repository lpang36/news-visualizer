...
def compute_edges():
  ...
  def plot(embeddings, labels, edges):
    ...
    for i, label in enumerate(labels):
      ...
      pylab.annotate(label, xy=(x, y), xytext=(5, 2), textcoords='offset points',
                     ha='right', va='bottom')
    for i in edges:
      pylab.plot([embeddings[i[0]][0],embeddings[i[1]][0]],[embeddings[i[0]][1],embeddings[i[1]][1]],lw=math.log(edges[i]))
    pylab.show()

  plot(embed, [a[1] for a in kws], edges)
